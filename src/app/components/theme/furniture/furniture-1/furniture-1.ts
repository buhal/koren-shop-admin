...
import { IBanners, IFurnitureOne } from '../../../../shared/interface/theme.interface';
...
export class Furniture1 {
  ...
  home_page$: Observable<IFurnitureOne> = inject(Store).select(ThemeState.homePage);
  ...
  public page_data: IFurnitureOne;
  ...
  constructor() {
    ...
    this.form = new FormGroup({
      ...
      slug: new FormControl('furniture_one'),
    });
  }
  ...
  ngOnInit() {
    ...
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'furniture_one' }));
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: IFurnitureOne): number[] {
    const result: number[] = [];
