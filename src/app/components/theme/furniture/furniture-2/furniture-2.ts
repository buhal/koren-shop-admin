...
import { IBanners, IFurnitureTwo } from '../../../../shared/interface/theme.interface';
...
@Component({
  ...
  templateUrl: './furniture-2.html',
  styleUrl: './furniture-2.scss',
})
export class Furniture2 {
  private store = inject(Store);
  formBuilder = inject(FormBuilder);
  private renderer = inject(Renderer2);
  private document = inject<Document>(DOCUMENT);

  home_page$: Observable<IFurnitureTwo> = inject(Store).select(ThemeState.homePage);
  ...
  public page_data: IFurnitureTwo;
  ...
  ngOnInit() {
    ...
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'furniture_two' }));
  ...
  patchForm() {
    this.home_page$.subscribe(homePage => {
      ...
      this.grid_banner = homePage?.content?.grid_banner?.banners.length
        ? homePage?.content?.grid_banner?.banners.length
        : 0;
          ...
          product_list_2: {
            ...
            products: {
              ...
              product_banner: {
                banner_1: {
                  image_url:
                    homePage?.content?.product_list_2?.products?.product_banner?.banner_1
                      ?.image_url,
                  status:
                    homePage?.content?.product_list_2?.products?.product_banner?.banner_1?.status,
                  redirect_link: {
                    link: homePage?.content?.product_list_2?.products?.product_banner?.banner_1
                      ?.redirect_link?.link,
                    link_type:
                      homePage?.content?.product_list_2?.products?.product_banner?.banner_1
                ...
                banner_2: {
                  image_url:
                    homePage?.content?.product_list_2?.products?.product_banner?.banner_2
                      ?.image_url,
                  status:
                    homePage?.content?.product_list_2?.products?.product_banner?.banner_2?.status,
                  redirect_link: {
                    link: homePage?.content?.product_list_2?.products?.product_banner?.banner_2
                      ?.redirect_link?.link,
                    link_type:
                      homePage?.content?.product_list_2?.products?.product_banner?.banner_2
                        ?.redirect_link?.link_type,
      ...
      homePage?.content?.grid_banner?.banners?.forEach((banner: IBanners) =>
        this.gridArray.push(
          this.formBuilder.group({
            redirect_link: new FormGroup({
              link: new FormControl(banner?.redirect_link?.link),
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: IFurnitureTwo): number[] {
    const result: number[] = [];
      ...
      if (Array.isArray(value)) {
        ...
        for (const [key, nested] of Object.entries(value)) {
          if (
            ...
            nested.every((item): item is number => typeof item === 'number')
          ) {
            result.push(...nested);
          } else {
            traverse(nested);
