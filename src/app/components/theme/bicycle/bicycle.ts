...
import { IBanners, IBicycle } from '../../../shared/interface/theme.interface';
...
export class Bicycle {
  ...
  home_page$: Observable<IBicycle> = inject(Store).select(ThemeState.homePage);
  ...
  ngOnInit() {
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'bicycle' }));
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: IBicycle): number[] {
    const result: number[] = [];
