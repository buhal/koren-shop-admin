...
import { IBag, IBanners } from '../../../shared/interface/theme.interface';
...
export class Bag {
  ...
  home_page$: Observable<IBag> = inject(Store).select(ThemeState.homePage);
  ...
  ngOnInit() {
    ...
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'bag' }));
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: IBag): number[] {
    const result: number[] = [];
