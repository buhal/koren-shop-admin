...
import { IBanners, IFashionOne } from '../../../../shared/interface/theme.interface';
...
export class Fashion1 {
  ...
  home_page$: Observable<IFashionOne> = inject(Store).select(ThemeState.homePage);
  ...
  constructor() {
    ...
    this.form = new FormGroup({
      ...
      slug: new FormControl('fashion_one'),
    });
  }

  ngOnInit() {
    const blogs$ = this.store.dispatch(new GetBlogsAction({ status: 1 }));
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'fashion_one' }));
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: IFashionOne): number[] {
    const result: number[] = [];
