...
import { IBanners, IFashionThree } from '../../../../shared/interface/theme.interface';
...
export class Fashion3 {
  ...
  home_page$: Observable<IFashionThree> = inject(Store).select(ThemeState.homePage);
  ...
  public page_data: IFashionThree;
  public banner = 1;
  ...
  constructor() {
    ...
    this.form = new FormGroup({
      ...
      slug: new FormControl('fashion_three'),
    });
  }

  ngOnInit() {
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'fashion_three' }));
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: IFashionThree): number[] {
    const result: number[] = [];
