...
import { IBanners, IFlower } from '../../../shared/interface/theme.interface';
...
export class Flower {
  ...
  home_page$: Observable<IFlower> = inject(Store).select(ThemeState.homePage);
  ...
  constructor() {
    ...
    this.form = new FormGroup({
      ...
      slug: new FormControl('flower'),
    });
  }

  ngOnInit() {
    const blogs$ = this.store.dispatch(new GetBlogsAction({ status: 1 }));
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'flower' }));
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: IFlower): number[] {
    const result: number[] = [];
