...
import { IBanners, IChristmas } from '../../../shared/interface/theme.interface';
...
export class Christmas {
  ...
  home_page$: Observable<IChristmas> = inject(Store).select(ThemeState.homePage);
  ...
  constructor() {
    ...
    this.form = new FormGroup({
      ...
      slug: new FormControl('christmas'),
    });
  }

  ngOnInit() {
    const blogs$ = this.store.dispatch(new GetBlogsAction({ status: 1 }));
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'christmas' }));
  ...
  patchForm() {
    this.home_page$.subscribe(homePage => {
      ...
      this.selectedCategories = homePage?.content?.category_product_1?.category_ids || [];
      this.selectedCategories2 = homePage?.content?.category_product_2?.category_ids || [];
      ...
      this.form.patchValue({
        content: {
          ...
          category_product_1: {
            tag: homePage?.content?.category_product_1?.tag,
            title: homePage?.content?.category_product_1?.title,
            category_ids: homePage?.content?.category_product_1?.category_ids,
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: IChristmas): number[] {
    const result: number[] = [];
