...
import { IBanners, IKids } from '../../../shared/interface/theme.interface';
...
export class Kids {
  ...
  home_page$: Observable<IKids> = inject(Store).select(ThemeState.homePage);
  ...
  ngOnInit() {
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'kids' }));
    const brand$ = this.store.dispatch(new GetBrandsAction({ status: 1 }));
    this.store.dispatch(new GetCategoriesAction({ status: 1, type: 'product' }));
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: IKids): number[] {
    const result: number[] = [];
