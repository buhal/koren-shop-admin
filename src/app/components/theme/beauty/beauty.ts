...
import { IBanners, IBeauty } from '../../../shared/interface/theme.interface';
...
export class Beauty {
  ...
  home_page$: Observable<IBeauty> = inject(Store).select(ThemeState.homePage);
  ...
  ngOnInit() {
    ...
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'beauty' }));
  ...
  patchForm() {
    this.home_page$.subscribe(homePage => {
      ...
      homePage?.content?.about_us?.services?.banners?.forEach((banner: IBanners) =>
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: IBeauty): number[] {
    const result: number[] = [];
