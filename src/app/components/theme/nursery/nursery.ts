...
import { IBanners, INursery } from '../../../shared/interface/theme.interface';
...
export class Nursery {
  ...
  home_page$: Observable<INursery> = inject(Store).select(ThemeState.homePage);
  ...
  ngOnInit() {
    ...
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'nursery' }));
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: INursery): number[] {
    const result: number[] = [];
