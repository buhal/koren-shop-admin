...
export class Marijuana {
  ...
  home_page$: Observable<IMarijuana> = inject(Store).select(ThemeState.homePage);
  ...
  constructor() {
    ...
    this.form = new FormGroup({
      ...
      slug: new FormControl('marijuana'),
    });
  }

  ngOnInit() {
    const blogs$ = this.store.dispatch(new GetBlogsAction({ status: 1 }));
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'marijuana' }));
  ...
  patchForm() {
    this.home_page$.subscribe(homePage => {
      ...
      this.form.patchValue({
        content: {
          ...
          offer_banner: {
            ...
            title: homePage?.content?.offer_banner?.title,
            tag: homePage?.content?.offer_banner?.tag,
            description: homePage?.content?.offer_banner?.description,
            banner: {
              banner_1: {
                status: homePage?.content?.offer_banner?.banner?.banner_1?.status,
                image_url: homePage?.content?.offer_banner?.banner?.banner_1?.image_url,
                redirect_link: {
                  link: homePage?.content?.offer_banner?.banner?.banner_1?.redirect_link?.link,
                  link_type:
                    homePage?.content?.offer_banner?.banner?.banner_1?.redirect_link?.link_type,
                  product_ids:
                    homePage?.content?.offer_banner?.banner?.banner_1?.redirect_link?.product_ids,
                },
              },
              banner_2: {
                status: homePage?.content?.offer_banner?.banner?.banner_2?.status,
                image_url: homePage?.content?.offer_banner?.banner?.banner_2?.image_url,
                redirect_link: {
                  link: homePage?.content?.offer_banner?.banner?.banner_2?.redirect_link?.link,
                  link_type:
                    homePage?.content?.offer_banner?.banner?.banner_2?.redirect_link?.link_type,
                  product_ids:
                    homePage?.content?.offer_banner?.banner?.banner_2?.redirect_link?.product_ids,
                },
              },
            },
          },
          details_section: {
            status: homePage?.content?.details_section?.status,
          },
          category_product: {
            ...
            image_url: homePage?.content?.category_product?.image_url,
      ...
      this.detailsSectionArray.clear();
      homePage?.content?.details_section?.banners?.forEach((banner: IBanners) =>
        this.detailsSectionArray.push(
          this.formBuilder.group({
  ...
  get detailsSectionArray(): FormArray {
    return this.form.get('content.details_section.banners') as FormArray;
  }
  ...
  removeDetailsSection(index: number) {
    if (this.homeBannersArray.length <= 1) return;
    this.detailsSectionArray.removeAt(index);
  }
  ...
  selectDetailsSectionArray(url: string, index: number) {
    this.detailsSectionArray
      .at(index)
      .get('image_url')
      ?.setValue(url ? url : null);
  }
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: IMarijuana): number[] {
    const result: number[] = [];
