...
export class Vegetables4 {
  ...
  home_page$: Observable<IVegetablesFour> = inject(Store).select(ThemeState.homePage);
  ...
  public page_data: IVegetablesFour;
  ...
  ngOnInit() {
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'vegetables_four' }));
  ...
  patchForm() {
    this.home_page$.subscribe(homePage => {
      ...
      homePage?.content?.offer_banner_2?.banners?.forEach((banner: IBanners) =>
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: IVegetablesFour): number[] {
    const result: number[] = [];
