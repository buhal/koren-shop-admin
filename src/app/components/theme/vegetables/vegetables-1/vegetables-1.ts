...
import { IBanners, IVegetablesOne } from '../../../../shared/interface/theme.interface';
...
export class Vegetables1 {
  ...
  home_page$: Observable<IVegetablesOne> = inject(Store).select(ThemeState.homePage);
  ...
  public page_data: IVegetablesOne;
  ...
  constructor() {
    ...
    this.form = new FormGroup({
      ...
      slug: new FormControl('vegetables_one'),
    });
  }

  ngOnInit() {
    const blogs$ = this.store.dispatch(new GetBlogsAction({ status: 1 }));
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'vegetables_one' }));
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: IVegetablesOne): number[] {
    const result: number[] = [];
