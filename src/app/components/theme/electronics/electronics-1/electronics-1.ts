...
import { IBanners, IElectronicOne } from '../../../../shared/interface/theme.interface';
...
@Component({
  ...
  templateUrl: './electronics-1.html',
  styleUrl: './electronics-1.scss',
})
export class Electronics1 {
  private store = inject(Store);
  formBuilder = inject(FormBuilder);
  private renderer = inject(Renderer2);
  private document = inject<Document>(DOCUMENT);

  home_page$: Observable<IElectronicOne> = inject(Store).select(ThemeState.homePage);
  ...
  public page_data: IElectronicOne;
  ...
  constructor() {
    ...
    this.form = new FormGroup({
      ...
      slug: new FormControl('electronics_one'),
    });
  }
  ...
  ngOnInit() {
    ...
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'electronics_one' }));
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: IElectronicOne): number[] {
    const result: number[] = [];
