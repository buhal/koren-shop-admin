...
import { IElectronicTwo } from '../../../../shared/interface/theme.interface';
...
@Component({
  ...
  templateUrl: './electronics-2.html',
  styleUrl: './electronics-2.scss',
})
export class Electronics2 {
  private store = inject(Store);
  formBuilder = inject(FormBuilder);
  private renderer = inject(Renderer2);
  private document = inject<Document>(DOCUMENT);

  home_page$: Observable<IElectronicTwo> = inject(Store).select(ThemeState.homePage);
  ...
  public page_data: IElectronicTwo;
  ...
  ngOnInit() {
    ...
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'electronics_two' }));
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: IElectronicTwo): number[] {
    const result: number[] = [];
