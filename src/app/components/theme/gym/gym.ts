...
import { IBanners, IGymSection } from '../../../shared/interface/theme.interface';
...
export class Gym {
  ...
  home_page$: Observable<IGymSection> = inject(Store).select(ThemeState.homePage);
  ...
  constructor() {
    ...
    this.form = new FormGroup({
      content: new FormGroup({
        ...
        parallax_product: new FormGroup({
          tag: new FormControl(''),
          title: new FormControl(''),
          description: new FormControl(''),
          product_ids: new FormControl([]),
      ...
      slug: new FormControl('gym'),
    });
  }

  ngOnInit() {
    const blogs$ = this.store.dispatch(new GetBlogsAction({ status: 1 }));
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'gym' }));
    ...
    this.home_page$.subscribe(homePage => {
      ...
      this.form.patchValue({
        content: {
          ...
          parallax_product: {
            tag: homePage?.content?.parallax_product?.tag,
            title: homePage?.content?.parallax_product?.title,
            description: homePage?.content?.parallax_product?.description,
            image_url: homePage?.content?.parallax_product?.image_url,
            product_ids: homePage?.content?.parallax_product?.product_ids,
            status: homePage?.content?.parallax_product?.status,
          },
          featured_blogs: {
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: IGymSection): number[] {
    const result: number[] = [];
