...
import { IBanners, IWatch } from '../../../shared/interface/theme.interface';
...
export class Watch {
  ...
  home_page$: Observable<IWatch> = inject(Store).select(ThemeState.homePage);
  ...
  ngOnInit() {
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'watch' }));
  ...
  concatDynamicProductKeys(obj: IWatch): number[] {
    const result: number[] = [];
