...
import { IBooks, IBanners } from '../../../shared/interface/theme.interface';
...
export class Books {
  ...
  home_page$: Observable<IBooks> = inject(Store).select(ThemeState.homePage);
  ...
  ngOnInit() {
    ...
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'books' }));
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: IBooks): number[] {
    const result: number[] = [];
