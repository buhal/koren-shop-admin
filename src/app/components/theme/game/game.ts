...
import { IBanners, IGame } from '../../../shared/interface/theme.interface';
...
export class Game {
  ...
  home_page$: Observable<IGame> = inject(Store).select(ThemeState.homePage);
  ...
  constructor() {
    ...
    this.form = new FormGroup({
      ...
      slug: new FormControl('game'),
    });
  }

  ngOnInit() {
    const blogs$ = this.store.dispatch(new GetBlogsAction({ status: 1 }));
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'game' }));
      ...
      this.form.patchValue({
        content: {
          ...
          category_product: {
            ...
            description: homePage?.content?.category_product?.description,
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: IGame): number[] {
    const result: number[] = [];
