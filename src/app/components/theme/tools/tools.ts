...
import { IBanners, ITools } from '../../../shared/interface/theme.interface';
...
export class Tools {
  ...
  home_page$: Observable<ITools> = inject(Store).select(ThemeState.homePage);
  ...
  constructor() {
    ...
    this.form = new FormGroup({
      ...
      slug: new FormControl('tools'),
    });
  }

  ngOnInit() {
    const blogs$ = this.store.dispatch(new GetBlogsAction({ status: 1 }));
    const home_page$ = this.store.dispatch(new GetHomePageAction({ slug: 'tools' }));
    ...
    this.home_page$.subscribe(homePage => {
      ...
      this.form.patchValue({
        content: {
          ...
          categories: {
            description: homePage?.content?.categories?.description,
  ...
  // Merge Products Ids
  concatDynamicProductKeys(obj: ITools): number[] {
    const result: number[] = [];
